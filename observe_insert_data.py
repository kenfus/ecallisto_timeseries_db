...
from bulk_load_to_database_between_dates import add_instruments_from_paths_to_database, add_specs_from_paths_to_database, create_dict_of_instrument_paths
...
def create_observer_for_day(date, event_handler):
    path = os.path.join(FILES_LOCAL_PATH, str(date.year), str(date.month).zfill(2), str(date.day).zfill(2))
...
def observe_days(days_to_observe):
    ### 
    date_range = pd.date_range(pd.Timestamp.now() - pd.DateOffset(days=days_to_observe), pd.Timestamp.now(), freq="D")
    LOGGER.info(f"Checking for new data in the last {days_to_observe} days. First day: {date_range[0].date()}, last day: {date_range[-1].date()}.")
    LOGGER.info(f"Will start observing {len(date_range)} days and thus starting {len(date_range)} observers.")
    # Observers
    observers = []
    ...
    try:
        ...
        while True:
            time.sleep(1)
            now = datetime.now()
            if now.month > current_date.month or now.year > current_date.year or now.day > current_date.day:
                # Create new observer
                observer = create_observer_for_day(current_date, event_handler)
                if observer == None:
                    continue
                observers.append(observer)
                LOGGER.info(f"New day has started. Stopping observers and creating new observers for {now.year}-{now.month}.")
                LOGGER.info(f"Created new observer for {current_date}. There are now {len(observers)} observers running.")
                # Stop the observers
